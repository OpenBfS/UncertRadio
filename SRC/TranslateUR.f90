...
subroutine ListstoreTranslate
     ...
     ! with more than 1 parameter are not yet stored ina liststore.
...
use UR_Gleich,        only: vdopt,ndopt,absrel,vcovcor,GrFormat,MDtyp,incall,vdoptfull, &
                            vdopt_pname
...
use UR_variables,         only: langg
use gtk,                  only: gtk_widget_hide, gtk_window_set_title, gtk_widget_set_visible, &
                                gtk_widget_set_tooltip_text,gtk_label_set_text,gtk_widget_set_name

use UR_gtk_variables,     only: Notebook_labeltext,clobj,nclobj
use Top,                  only: idpt,LFU
use Rout,                 only: WDPutLabelString,WDPutLabelStringBold,SetTooltipText,     &
                                WDPutTreeViewColumnLabel,WDGetTreeViewColumnLabel,  &
                                WDSetComboboxAct,WDGetLabelString
use g,                    only: g_strip_context,g_dgettext
...
if(langg == 'FR') call WDSetComboboxAct('comboboxLangg',3)
  ...
  do i=1,6
    ...
    call gtk_label_set_text(widget,trim(FLTU(Notebook_labeltext(i)))//c_null_char)
  end do
...
!                                      trim(FLTU('Formule de courbe de décroissance:')) // c_null_char)

if(langg == 'DE') call gtk_window_set_title(idpt('dialogDecayModel'),  &
                                      trim(FLTU('Linfit: Vorgaben zur Ausführung der Anpassung:')) // c_null_char)
if(langg == 'EN') call gtk_window_set_title(idpt('dialogDecayModel'),  &
                                      'Linfit: Specifications for running the adjustment:' // c_null_char)
if(langg == 'FR') call gtk_window_set_title(idpt('dialogDecayModel'),  &
                                      trim(FLTU('Linfit : Spécifications pour l''exécution de l''ajustement :')) // c_null_char)
...
if(langg == 'DE') call gtk_window_set_title(idpt('dialog_BinPoi'),  &
                                      trim(FLTU('Binom+Poisson - Parameter:')) // c_null_char)
if(langg == 'EN') call gtk_window_set_title(idpt('dialog_BinPoi'),  &
                                      trim(FLTU('Binom+Poisson - parameters:')) // c_null_char)
if(langg == 'FR') call gtk_window_set_title(idpt('dialog_BinPoi'),  &
                                      trim(FLTU('Binom+Poisson - paramètre:')) // c_null_char)
