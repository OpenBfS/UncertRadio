...
character(:),allocatable  :: fnG,fncsv, str1,tpart,str2,text18,wpath,text19,cfnam,fname_auto_sep
...
allocate(character(len=800) :: fnG,fncsv, str1,tpart,str2,wpath,cfnam,fname_auto_sep)
...
if(.not.outsepar) then
  ...
  if(i1 > 0) then
    if(index(fname,':') == 0) then
      fname_auto_sep = TRIM(wpath) // trim(fname(1:i1-1))//'_res.csv'
    else
...
28      continue
  OPEN (21,FILE=TRIM(fncsv), STATUS='unknown',POSITION='Append',IOSTAT=ios)
  if(ios == 2) OPEN (21,FILE=TRIM(fncsv), STATUS='new',POSITION='Append',IOSTAT=ios)
  ...
  IF(.not.lexist) then
    OPEN (21,FILE=TRIM(fncsv), STATUS='unknown',POSITION='Append',IOSTAT=ios)
      ...
      write(66,*) 'fncsv=',trim(fncsv)
       close (21)
    OPEN (21,FILE=TRIM(fncsv), STATUS='old',IOSTAT=ios)
    if(ios /= 0) call errwrite(.true.,21,'  ',trim(fncsv),ios,resp)
       write(66,*) 'ios=',int(ios,2),' ifehl=',int(ifehl,2)
    if(ifehl == 1) return
    if(ios == 0) then
...
if(outsepar) then
  ...
  if(neg == 1) then
    ...
    call errwrite(.false.,18,trim(btext),fncsv,ios,resp)
    if(ifehl == 1) return
  end if
  ...
  call errwrite(.false.,18,text18,fncsv,ios,resp)
  if(ifehl == 1) return
  goto 40
end if
...
deallocate(fnG,fncsv, str1,tpart,str2,text18,wpath,text19,cfnam)
...
integer(c_int)          :: resp
logical                 :: lexist
character(:),allocatable  :: str1, tpart,wpath, str2,text16,cfnam
!---------------------------------------------------------------------------------------
ctr = sListSeparator         ! ';'
ifehl = 0

allocate(character(len=900)  ::  text16)
allocate(character(len=1100) ::  wpath, str2,tpart)
...
cfnam = TRIM(wpath) // 'UR-Saved-Results.csv'    ! unit 16 is used for writing
  !  finfo(9): file size in kB
  !  finfo(1): file size in Byte, or -1, if not existing
...
deallocate(text16, wpath, str2,tpart, str1, cfnam)
