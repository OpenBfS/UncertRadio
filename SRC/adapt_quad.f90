...
res3la(2) = res3la(3)
res3la(3) = result
100 abserr = MAX(abserr, 5.0_dp*epmach*ABS(result))
RETURN
END SUBROUTINE qelg
...
uflow = dpmpar(2)
oflow = dpmpar(3)
          !  write(28,*) 'epmach=',epmach,' uflow=',uflow,' oflow=',oflow
...
elist(1) = 0.0_dp
ier = 6
IF (epsabs < 0.0_dp .OR. epsrel < 0.0_dp) GO TO 999
ier = 0
rerr = MAX(epsrel, 50.0_dp*epmach)
...
IF (limit == 1) ier = 1
IF (ier /= 0 .OR. (abserr <= errbnd .AND. abs(abserr-resabs) > epsilon(1._dp)) .OR.  &
                                   abs(abserr) < epsilon(1._dp)) GO TO 999
...
!      AND ERROR AND TEST FOR ACCURACY.
  ...
  IF (abs(defab1 - error1) < epsilon(1._dp) .OR. abs(defab2 - error2) < 1._dp) GO TO 15
  IF (ABS(rlist(maxerr) - area12) > 0.1E-04_dp*ABS(area12)  &
        .OR. erro12 < 0.99_dp*errmax) GO TO 10
...
!      AT A POINT OF THE INTEGRATION RANGE.
  ...
  IF (MAX(ABS(a1),ABS(b2)) <= t*(ABS(a2) + 1.E+03_dp*uflow)) ier = 4
...
!      MORE CAUTIOUS EXTRAPOLATION THAN IN THE ORIGINAL DQAGSE ROUTINE
  ...
  40 IF (ierro == 3 .OR. erlarg <= 0.3_dp*ertest) GO TO 60
...
100 IF (abs(abserr - oflow) < epsilon(1._dp)) GO TO 115
IF (ier + ierro == 0) GO TO 110
IF (ierro == 3) abserr = abserr + correc
IF (ier == 0) ier = 3
IF (abs(result) > epsilon(1._dp).AND. abs(area) > epsilon(1._dp)) GO TO 105
IF (abserr > errsum) GO TO 115
IF (abs(area) < epsilon(1._dp)) GO TO 130
GO TO 110
...
defabs*0.1E-01_dp) GO TO 130
IF(0.1E-01_dp > (result/area) .OR. (result/area) > 0.1E+03_dp  &
...
CALL qxrul (f, a, b, resg, resabs, resasc, k0, k1, vr, vs, lr, ls)
    ! write(28,*) 'qxlqm: nach qxrul: resg, resabs, resasc=',resg, resabs, resasc
errold = oflow
t = 10.0_dp*epmach
DO k = k1, k2
  CALL qxrul (f, a, b, resk, resabs, resasc, k, k1, vr, vs, lr, ls)
      ! write(28,*) 'qxlqm: nach qxrul k=',k,' resk, resabs, resasc=',resk, resabs, resasc
  result = resk
  abserr = ABS(resk - resg)
  IF (abs(resasc) > epsilon(1._dp) .AND. abs(abserr) > epsilon(1._dp))  &
  abserr = resasc*MIN(1.0_dp,(200.0_dp*abserr/resasc)**1.5_dp)
  IF (resabs > uflow/t) abserr = MAX(t*resabs,abserr)
  resg = resk
  IF (abserr > errold*0.16_dp) EXIT
  IF (abserr < 1000.0_dp*epmach*resabs) CYCLE
  errold = abserr
END DO
...
!------------------------
REAL (dp) :: xx(21) = (/ 0._dp,                      .25000000000000000000E+00_dp, &
                       .50000000000000000000E+00_dp, .75000000000000000000E+00_dp, &
                       .87500000000000000000E+00_dp, .93750000000000000000E+00_dp, &
                       .10000000000000000000E+01_dp, .37500000000000000000E+00_dp, &
                       .62500000000000000000E+00_dp, .96875000000000000000E+00_dp, &
                       .12500000000000000000E+00_dp, .68750000000000000000E+00_dp, &
                       .81250000000000000000E+00_dp, .98437500000000000000E+00_dp, &
                       .18750000000000000000E+00_dp, .31250000000000000000E+00_dp, &
                       .43750000000000000000E+00_dp, .56250000000000000000E+00_dp, &
                       .84375000000000000000E+00_dp, .90625000000000000000E+00_dp, &
                       .99218750000000000000E+00_dp /)
REAL (dp) :: ww(52) = (/     &
1.303262173284849021810473057638590518409112513421E-1_dp, 2.390632866847646220320329836544615917290026806242E-1_dp,  &
2.630626354774670227333506083741355715758124943143E-1_dp, 2.186819313830574175167853094864355208948886875898E-1_dp,  &
2.757897646642836865859601197607471574336674206700E-2_dp, 1.055750100538458443365034879086669791305550493830E-1_dp,  &
1.571194260595182254168429283636656908546309467968E-2_dp, 1.298751627936015783241173611320651866834051160074E-1_dp,  &
2.249996826462523640447834514709508786970828213187E-1_dp, 1.680415725925575286319046726692683040162290325505E-1_dp,  &
1.415567675701225879892811622832845252125600939627E-1_dp, 1.006482260551160175038684459742336605269707889822E-1_dp,  &
2.510604860724282479058338820428989444699235030871E-2_dp, 9.402964360009747110031098328922608224934320397592E-3_dp,  &
5.542699233295875168406783695143646338274805359780E-2_dp, 9.986735247403367525720377847755415293097913496236E-2_dp,  &
4.507523056810492466415880450799432587809828791196E-2_dp, 6.300942249647773931746170540321811473310938661469E-2_dp,  &
1.261383225537664703012999637242003647020326905948E-1_dp, 1.273864433581028272878709981850307363453523117880E-1_dp,  &
8.576500414311820514214087864326799153427368592787E-2_dp, 7.102884842310253397447305465997026228407227220665E-2_dp,  &
5.026383572857942403759829860675892897279675661654E-2_dp, 4.683670010609093810432609684738393586390722052124E-3_dp,  &
1.235837891364555000245004813294817451524633100256E-1_dp, 1.148933497158144016800199601785309838604146040215E-1_dp,  &
1.252575774226122633391477702593585307254527198070E-2_dp, 1.239572396231834242194189674243818619042280816640E-1_dp,  &
2.501306413750310579525950767549691151739047969345E-2_dp, 4.915957918146130094258849161350510503556792927578E-2_dp,  &
2.259167374956474713302030584548274729936249753832E-2_dp, 6.362762978782724559269342300509058175967124446839E-2_dp,  &
9.950065827346794643193261975720606296171462239514E-2_dp, 7.048220002718565366098742295389607994441704889441E-2_dp,  &
6.512297339398335645872697307762912795346716454337E-2_dp, 3.998229150313659724790527138690215186863915308702E-2_dp,  &
3.456512257080287509832054272964315588028252136044E-2_dp, 2.212167975884114432760321569298651047876071264944E-3_dp,  &
8.140326425945938045967829319725797511040878579808E-2_dp, 6.583213447600552906273539578430361199084485578379E-2_dp,  &
2.592913726450792546064232192976262988065252032902E-2_dp, 1.187141856692283347609436153545356484256869129472E-1_dp,  &
5.999947605385971985589674757013565610751028128731E-2_dp, 5.500937980198041736910257988346101839062581489820E-2_dp,  &
5.264422421764655969760271538981443718440340270116E-3_dp, 1.533126874056586959338368742803997744815413565014E-2_dp,  &
3.527159369750123100455704702965541866345781113903E-2_dp, 5.000556431653955124212795201196389006184693561679E-2_dp,  &
5.744164831179720106340717579281831675999717767532E-2_dp, 1.598823797283813438301248206397233634639162043386E-2_dp,  &
2.635660410220884993472478832884065450876913559421E-2_dp, 1.196003937945541091670106760660561117114584656319E-2_dp /)
!------------------------
k = ke + 1
is = istart(k)
ks = length(k)
...
aa = xl + bb
       ...
       ! write(28,*) 'l1,l2=',l1,l2
y = 0.0_dp
DO i = 1, ks
  c = bb*xx(i)
  IF (i > l1) fv1(i) = f(aa + c)
  IF (i > l2) fv2(i) = f(aa - c)
       ! write(28,*) 'l1,l2=',l1,l2,' fv1=',fv1(i),' fv2=',fv2(i),' aa-c=',aa-c
  j = is + i
...
r(8) =  f(centr + dlen*0.375_dp)
r(9) =  f(centr + dlen*0.625_dp)
r(10) = f(centr + dlen*0.96875_dp)
lr = 10
IF (lz /= 11) s(4) = f(centr - dlen*0.75_dp)
IF (lz == 11) s(4) = z(11)
s(5) =  f(centr - dlen*0.875_dp)
s(6) =  f(centr - dlen*0.9375_dp)
s(8) =  f(centr - dlen*0.375_dp)
s(9) =  f(centr - dlen*0.625_dp)
s(10) = f(centr - dlen*0.96875_dp)
ls = 10
RETURN
