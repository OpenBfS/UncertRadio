...
if (wpunix) then
    DirectorySeparator = '/'
    WRITE(0,*) 'Operating System: Linux'
else
    DirectorySeparator = '\'
    WRITE(0,*) 'Operating System: Windows'
endif

#ifdef GITHASH
  UR_GIT_Version = GITHASH
  WRITE(0,*) "Git Version: "//trim(UR_GIT_Version)
#endif
...
! open(196,file=trim(actpath) // "Fort196.txt")
open(15,file=trim(actpath) // "Fort15.txt")
! open(69,file=trim(actpath) // "Fort69.txt")

  !  open(63,file=trim(actpath) // "Fort63.txt",position="Append")
  ...
  if(TRIM(str1) == 'AUTO' .or. TRIM(str1) == 'AUTOSEP') THEN
    ...
    do i=LEN_TRIM(fname_getarg),1,-1
      IF(fname_getarg(i:i) == '/') fname_getarg(i:i) = '\'
           ...
           ! write(66,*) 'fname_getarg=',trim(fname_getarg),' sample_ID=',trim(sample_ID)
