...
.. note::
   **Platform compatibility**: The command line examples in this section
   are **Windows-only**. On Linux and macOS, the syntax for command line
   arguments is different (no "Start" prefix, different path separators).
   The Excel batch mode examples (see :ref:`batch mode processing with an excel application`)
   are **Windows-only** and require Microsoft Excel with VBA support.
   LibreOffice Calc and other spreadsheet applications are not supported.
