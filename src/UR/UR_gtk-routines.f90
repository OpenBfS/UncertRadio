...
use, intrinsic :: iso_c_binding,      only: c_int,c_null_char,c_null_ptr,c_associated, &
                                            c_f_pointer
...
use gtk,                  only: gtk_entry_set_text
! use g,                    only: g_signal_handler_block,g_signal_handler_unblock
...
string = FormatNumStr(trim(string))
   ! write(66,*) 'widget=',widget,'   string=',trim(string)
...
call hl_gtk_radio_menu_group_set_select(group, indx)
       ...
       ! call WDGetSelRadioMenu(wstr, j)
       ! write(66,*) 'putselradiomenu: k=',k,' ausgelesene option-number  j: ',j
item_setintern = .false.
...
group = gtk_radio_menu_item_get_group(Widget)
          !write(66,*) "kEGr_radio_group:",UR_win%kEGr_radio_group, "indx=",indx
          !write(66,*) "menuradio_group:",menuradio_group
...
!
use gtk,                     only: GTK_BUTTONS_OK, GTK_BUTTONS_Yes_NO,gtk_RESPONSE_YES, &
                                 & gtk_RESPONSE_No, gtk_RESPONSE_CANCEL, GTK_RESPONSE_APPLY, &
                                   GTK_MESSAGE_WARNING
USE UR_Variables
...
use UR_gtk_variables, only: tvnames,tvcolindex,tv_colwidth_digits,  &
                            scrwidth_max, scrwidth_min,scrheight_max, scrheight_min,monitorUR
