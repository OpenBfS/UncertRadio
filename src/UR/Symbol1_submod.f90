...
use UR_gtk_variables,   only: consoleout_gtk,contrast_mode
...
integer(4), parameter :: nfus = 17      ! number of primary mathematical functions

integer(4)               :: i,i1,n,k,i2,nfx,j,ios,mfd,k2,jh
integer(4)               :: resp,ngmax,nrsum,fkzahl,ix,j0,imax
integer(4)               :: ifd,nzk,nzlast
...
integer(4),allocatable   :: ivpos(:,:),ivlen(:,:),ivanz(:)
integer(4)               :: ifehlps,idel,kx,nfd,kLL,kRR
integer(4)               :: icp_used(nmumx+nabmx), i_1,i_2 ,i10,js,k1len
integer(4)               :: kk1,ivsort(50)
integer(4)               :: nfd1,nfd2,nip,ipos(50),kstt
real(rn)                 :: zahl
...
integer(4)               :: ncstr,nsyform,ihg,neli
LOGICAL                  :: Sprot,test
character(:),allocatable :: xstr
character(len=3)         :: ccc
integer(4),allocatable   :: ivpos1(:)
...
do i=1,ngrs
  ...
  call WTreeViewPutStrCell('treeview1', 3, i, xstr)
      ...
      if(.not.contrast_mode) call WTreeViewSetColorRow('treeview1',i, '#FFFFFF')         ! white
      if(contrast_mode) call WTreeViewSetColorRow('treeview1',i, "#1D1D1D")         ! contrast of white
  ...
  IF(ngrs_CP > 0 .and. mfd == 0 .and. len_trim(symbole(i)%s) > 0) THEN
    ...
    IF(ngrs_cp > 0) THEN
      ...
      if(.not.contrast_mode) call WTreeViewSetColorRow('treeview1',i, '#00FF48')         ! green
      if(contrast_mode) call WTreeViewSetColorRow('treeview1',i, '#1C891D')         ! green
    end if
...
integer(4)           :: i,k,j,resp,ncov0,ix,imax,nrs,nfd,ii
