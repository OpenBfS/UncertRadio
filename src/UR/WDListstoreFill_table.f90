...
type(c_ptr)                  :: liststore,renderer,tree
integer(4)                   :: nmax,ibc, nmaxk,k,ngrsx,kk,kkx,ncolorcols,itv
...
select case (mode)
  ...
  case (1,2,3)
    ...
    do i=1,nmax
      ...
      if(i > kkx) call g_value_set_string(pstring,'  '//c_null_char)
      call gtk_list_store_set_value(Liststore, c_loc(iter), 2_c_int, pstring)
      ...
      str1 = FLTU(str1)
                    ...
                    if(consoleout_gtk)  write(0,*) '          after FLTU'
      ...
      if(mode == 2 .and. ugr) then
        ...
        call gtk_list_store_set_value(Liststore,c_loc(iter), 5_c_int, pstring)
        ...
        if(i > kkx) call g_value_set_string(pstring,'  '//c_null_char)
        call gtk_list_store_set_value(Liststore, c_loc(iter), 6_c_int, pstring)
           if(consoleout_gtk) write(0,*) 'Column 7 ready'

        kkx = ubound(SDWert,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 8, SDWert(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 8, missingval, lmiss)

        kkx = ubound(HBreite,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 9, HBreite(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 9, missingval, lmiss)
        ...
        call gtk_list_store_set_value(Liststore, c_loc(iter), 9_c_int, pstring)
           if(consoleout_gtk) write(0,*) 'Column 10 ready'

        kkx = ubound(StdUnc,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 11, StdUnc(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 11, missingval, lmiss)
      ...
      if(mode == 3) then
        ...
        ! uncertainty budget:
        kkx = ubound(Messwert,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 5, Messwert(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 5, missingval, lmiss)
           if(consoleout_gtk) write(0,*) 'Spalte 6 fertig'

        kkx = ubound(StdUnc,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 6, StdUnc(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 6, missingval, lmiss)

        kkx = ubound(Sensi,dim=1)
        if(i <= kkx) call StoreStrDouble(Listname, 7, Sensi(i), lmiss)
        if(i > kkx) call StoreStrDouble(Listname, 7, missingval, lmiss)

        if(Ucontyp == 1) then
          kkx = ubound(Perc,dim=1)
          if(i <= kkx) call StoreStrDouble(Listname, 8, Perc(i), lmiss)
          if(i > kkx) call StoreStrDouble(Listname, 8, missingval, lmiss)
        elseif(Ucontyp == 2) then
          kkx = ubound(Ucontrib,dim=1)
          if(i <= kkx) call StoreStrDouble(Listname, 8, Ucontrib(i), lmiss)
          if(i > kkx) call StoreStrDouble(Listname, 8, missingval, lmiss)
        end if
  ...
  case (4)
    ...
    do i=1,ncov+25
      ...
      if(i <= ncov) then
        call StoreStrDouble(Listname, 6, CovarVal(i), lmiss)
      else
        call StoreStrDouble(Listname, 6, missingval, lmiss)
      end if
  ...
  case (5)
    ...
    do i=1, ndatmax         ! rows
      ...
      if(i <= numd) then
        call StoreStrDouble(Listname, 3, dmesszeit(i), lmiss)
        call StoreStrDouble(Listname, 4, dbimpulse(i), lmiss)
        call StoreStrDouble(Listname, 5, dbzrate(i), lmiss)
        call StoreStrDouble(Listname, 6, sdbzrate(i), lmiss)
        call StoreStrDouble(Listname, 7, d0messzeit(i), lmiss)
        call StoreStrDouble(Listname, 8, d0impulse(i), lmiss)
        call StoreStrDouble(Listname, 9, d0zrate(i), lmiss)
        call StoreStrDouble(Listname, 10, sd0zrate(i), lmiss)
        call StoreStrDouble(Listname, 11, dnetrate(i), lmiss)
        call StoreStrDouble(Listname, 12, sdnetrate(i), lmiss)
      else
        call StoreStrDouble(Listname, 3, missingval, lmiss)
        call StoreStrDouble(Listname, 4, missingval, lmiss)
        call StoreStrDouble(Listname, 5, missingval, lmiss)
        call StoreStrDouble(Listname, 6, missingval, lmiss)
        call StoreStrDouble(Listname, 7, missingval, lmiss)
        call StoreStrDouble(Listname, 8, missingval, lmiss)
        call StoreStrDouble(Listname, 9, missingval, lmiss)
        call StoreStrDouble(Listname, 10, missingval, lmiss)
        call StoreStrDouble(Listname, 11, missingval, lmiss)
        call StoreStrDouble(Listname, 12, missingval, lmiss)
      end if
  ...
  case (6)
    ...
    do i=1,nmax         ! rows
      ...
      call gtk_list_store_set_value(Liststore, c_loc(iter), 1_c_int, dintval)
      ...
      if(.true.) then
        if(.not.lmiss) call StoreStrDouble(Listname, 3, erg(i), lmiss)
        if(lmiss) call StoreStrDouble(Listname, 3, missingval, lmiss)
      end if

      if(.not.lmiss) call StoreStrDouble(Listname, 4, GnetRate(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 4, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 5, RateCB(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 5, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 6, RateBG(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 6,missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 7, SDRateBG(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 7, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 8, effi(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 8, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 9, SDeffi(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 9, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 10, pgamm(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 10, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 11, SDpgamm(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 11, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 12, fatt(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 12, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 13, SDfatt(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 13, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 14, fcoinsu(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 14, missingval, lmiss)
      if(.not.lmiss) call StoreStrDouble(Listname, 15, SDfcoinsu(i), lmiss)
      if(lmiss) call StoreStrDouble(Listname, 15, missingval, lmiss)
  ...
  case (7)
    ...
    do i=1,nmax         ! rows
      ...
      call gtk_list_store_set_value(Liststore, c_loc(iter), 0_c_int, dintval)
      if(i <= nkalpts) then
        call StoreStrDouble(Listname, 2, xkalib(i), lmiss)
        call StoreStrDouble(Listname, 3, uxkalib(i), lmiss)
        call StoreStrDouble(Listname, 4, ykalib(i), lmiss)
        call StoreStrDouble(Listname, 5, uykalib(i), lmiss)
      end if
      ncolorcols = 7
      if(progstart_on) then
        !vstr = "#FFFFFF"
...
subroutine StoreStrDouble(Listname, fcol, dvalue, lmiss)
...
if(.not.lmiss) then
  ...
  if(trim(Listname) == 'liststore_gspk1' .and. fcol == 2) frmt_w = '(f7.2)'
  if(trim(Listname) == 'liststore_covcor' .and. fcol == 6) frmt_w = trim(frmtc)
