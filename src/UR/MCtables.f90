...
subroutine MCtables(kqtyp)
    ...
    if(kqtyp == 1) then
        write(log_str,*) 'xmit1PE=',sngl(xmit1PE),' xsdvPE=',sngl(xsdvPE)
    ...
    IF(kqtyp <= 2) THEN
        WRITE(log_str,*) 'after MC-Loop, kqtyp=',int(kqtyp,2),' :'
        call logger(63, log_str)
        call logger(63, ' ')
        write(log_str,'(a)') '                         true value   MC value     MC/true       true SD      MC SD        MC/trueSD'
        call logger(63, log_str)
        ...
        do kk=1,ngrs+ncov+numd
            ...
            IF(ivtl(k) /= 4 .and. ivtl(k) /= 7) THEN
                ...
                if(kqtyp == 1) then
                    ...
                    write(log_str,'(i3,1x,a,T27,es11.4,2x,es11.4,2x,es11.4,1x,a1,1x,es11.4,2x,es11.4,2x,es11.4,1x,a1)') &
                ...
                write(log_str,*) int(k,2), SymboleG(k)%s,' analytical:',sngl(max(0.00001_rn, MesswertSV(k)+gdaf*GamDistAdd)), &
        ...
        if(kqtyp == 1 .and. FitDecay) then
            ...
            write(log_str,*) 'cov(Rn(1), Rn(2))=',sngl(covmw12),'   expected: ',sngl(dummy),'  parfixed=',parfixed
            call logger(63, log_str)
            ...
            write(log_str,*) ' mean(Mwnet(1)=',sngl(mw12),', its SD:',sngl(sdmw12),'  expected SD: ',sngl(sqrt(dummy))
            call logger(63, log_str)

        end if

        if(FitDecay .and. kqtyp == 2) then
            write(log_str,*) '  Net counting rates:'
            call logger(63, log_str)
            do k=1,numd
                utu = ' '
            ...
            dummy = sd0zrateSV(1)**TWO
            if(parfixed) dummy = dummy + afuncSV(1,3)*afuncSV(2,3)*( 0.025_rn**TWO + 0.05_rn**TWO )
            write(log_str,*) 'cov(Rn(1), Rn(2))=',sngl(covmw12),'   erwartet: ',sngl(dummy),'  parfixed=',parfixed
            call logger(63, log_str)
        end if
        write(log_str,*) 'Resulta(kEGr) obtained with Messwert-means of the MC Loop:',sngl(xmit2)
        call logger(63, log_str)
        if(FitDecay) then
            write(log_str,*) '   konstant_r0=',konstant_r0
            call logger(63, log_str)
        end if
        write(log_str,*) 'Ucomb-MC(kEGr): ',sngl(xsdv)
        call logger(63, log_str)
        IF(FitDecay) THEN
            write(log_str,*) ' xxxx Mean of the SDEV values from the single MC fits: ', sngl(xesdev1), &
                '  its StdDev:',sngl(xesdev2)
            call logger(63, log_str)
            write(log_str,*) ' xxxx Mean of the fpa values from the single MC fits : ', sngl(xemit1),  &
            ...
            if(kEGr == 1 .and. abs(fpa(1)*fpa(2)*fpa(3)) > EPS1MIN) then
                ...
                IF(knumEGr > 1) then
                    write(log_str,*) ' MC values for the covariance(Mw1, Mw2): ',sngl(ssxEG(1)),'  theoretical: ',sngl(xcovt)
                    call logger(63, log_str)
                end if
            end if
