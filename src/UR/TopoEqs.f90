...
recursive subroutine chains(kanf0,kanf,ksq)
    ...
    integer   ,intent(in)           :: kanf0    ! net count rate equation number,
    ! with the first call of chains
    integer   ,intent(in)           :: kanf     ! Equation number transition kanf --> ksq
    integer   ,intent(inout)        :: ksq      ! chain-number

    integer            :: nsd,n2,i,idy,klen,kk
...
subroutine chainseval(ksq1, ksq2)
    ...
    integer, intent(in) :: ksq1,ksq2   ! first and last chain number of a range of chains

    integer            :: i0a,i0b,i1a,i1b,i2a,i2b,i3a,i3b,i1c,i3c,nc,i,j,klen,kvor,nsd,kk,nn
    integer            :: nsd2,k,nkk,nminus,kk1,nsdops,i4,jj
    logical            :: arrive60
    character(len=512) :: log_str
...
!                                     ' krate(nc)=',krate(nc),' Eq(kvor)=',Rseite(kvor)%s
            ...
            if(krate(nc) == 0 .and. nsd > 0 .and. opnum(nsd) == 1) then     ! with operator '-'
                if(nRSsy(eqnum(nsd)) > 2) then
                    ...
                    if(nminus == nRSsy(eqnum(nsd))-1  .and. nkk >= 1) then
                        ...
                        if(prout .and. kk <= nab)  then
                            write(log_str, '(4(a,i0),a,a)') 'nkk=',nkk,' nminus=',nminus,' krate(nc)=', &
        ...
        if(kcnt(nc) > 0 .and. ktime(nc) > 0 .and. (ucase(symtyp(kk)%s) == 'M' .or. SumEval_fit) ) then
            write(log_str, '(3(a,i0))') 'goto 60 (sumeval?):    B1: kk=',kk,' kvor=',kvor,'  krate=',krate(nc)
            call logger(66, log_str)
            goto 60
        end if
...
subroutine SetPars(nc, nkrate, nktime, nkcnt, cRule)
    ...
    integer   ,intent(in)       :: nc,nkrate,nktime,nkcnt
