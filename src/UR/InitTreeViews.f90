subroutine InitTreeViews
    ...
    use UR_gtk_variables,        only: iter,contrast_mode,table_bg
    ...
    integer(4)                   :: rowmax,k,icol
    integer(4),allocatable       :: num(:),iuval(:)
    real(rn),allocatable         :: xval(:)
    type(charv),allocatable      :: sval(:)
    integer(c_int)               :: kcol
    integer(c_long)              :: i
    logical                      :: bcheck
    character(len=7)             :: vstr
