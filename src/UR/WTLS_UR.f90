...
if(.false.) then
  do i=1,numd
    WRITE(kunit,'(a,i3,a,es10.3,a,es10.3)') 'Y-Wert(',i,')=',s(i),'  U=',ds(i)
  end do
  WRITE(kunit,*)
  do i=1,numd*ma
    WRITE(kunit,'(a,i3,a,es10.3)') 'X-Wert(',i,')=',t(i)
  end do
end if
!-----------------------------------------------------------------------------------------
...
!!!! if(cofactlyt > zero) cofact = cofactlyt
...
call E7LSQ1UR(aG,ifit,nm,nn,nnr,t,s,ds,covarG,chisq,Tnstep,kunit, &
              nred,irun,printout)
IF(Tnstep < 0) THEN
  ifehl = 1
  WTLS_Wild = .true.
...
integer(4),INTENT(INOUT)   :: list(ma)     ! fit param: 1: yes ;  2: fixed; 3: not used
integer(4),INTENT(IN)      :: m            ! number of constraining equations, number of count rates
integer(4),INTENT(IN)      :: n            ! number measurements / measured values
integer(4),INTENT(INOUT)   :: nr           ! number of parameters
...
integer(4),INTENT(INOUT)   :: kunit        ! unit for printout
...
integer(4)         :: i,k,ir,kr,jx,nmk,k1,k2,kb1,kb2,k0
integer(4)         :: ik1,ik2,ifail,m1,m2
integer(4)         :: nnew,messk,messk2,nwh
integer(4)         :: nmk2,kjun,kk,k01,k02,kqt,i_arr(m),ijx_arr(m),list2(3)

real(rn)           :: rho(maxm),r,chisqr, cymin,cymax, tfit,schisqr
real(rn)           :: xred(ma)
!real(rn)           :: y(m*(mfit+1)),cy(m*(mfit+1),m*(mfit+1))
!real(rn)           :: cx(nred,nred)
...
integer(4), INTENT(IN)     :: nred              ! number of parameter to be fitted (<=nr)
...
integer(4), INTENT(IN OUT) :: list(nr)          ! fit param: 1: yes ;  2: fixed; 3: not used
...
integer(4), INTENT(IN OUT) :: nstep             ! number of iterations, or, if < 0: error indication
logical,intent(in)         :: printout

integer(4), PARAMETER   :: maxstp=100
! real(rn), PARAMETER :: epsiln=1.E-12_rn
real(rn), PARAMETER     :: epsiln=1.E-15  ! 1.E-16_rn      ! 6.8.2023

! real(rn), PARAMETER :: tt=5.E-13_rn
real(rn), PARAMETER     :: tt = 1.E-16_rn  ! 1.E-17_rn   ! tt=5.E-17_rn

integer(4)              :: i,l,istep,k,ired,kqt,nrepeat,m1,m2
real(rn)                :: rlst,xff
real(rn),allocatable    :: d(:),t(:,:),b(:,:),u(:,:), bb(:),uu(:)
integer(4),allocatable  :: i_arr(:)
LOGICAL                 :: ok,covmat
real(rn), Allocatable   :: fy(:,:),G(:,:),e(:,:),a2(:,:)       ! working areas

! COMMON /dasv03/ d,t,b,u    ! does not seem to be used by LSQGEN and routines therein
...
DO istep=1,nstep
  ...
  if(MCsim_on) xff = 0.5_rn
    ! IF( (istep > 1 .AND. ABS(r-rlst)*4._rn < epsiln*r+tt) .OR. istep == nstep ) THEN
  IF( (istep > 1 .AND. ABS(r-rlst)*xff < epsiln*r+tt) .OR. istep == nstep ) THEN
    nstep = istep
    IF(covmat) THEN
      ! compute matrix GB
     ...
     fy_2 = matmul(a2_2, matmul(cy, Transpose(a2_2)))
      ...
      CALL mtxchi(fy_2)              ! inverts matrix fy_2 by Cholesky decomposition
           if(printG) write(23,*) ' LSQGEN: after 2nd call MTXCHI (invert(fy_2)):  posdef=',posdef
              !if(.not.posdef) then
              !  ncofact = ncofact + 1
              !  cofact = cofact * (1._rn - 1.E-10_rn)
              !  cofactlyt = cofact
              !  write(23,'(a,L1,a,f15.12,3(a,i3))') 'LSQGEN: (2nd mtxchi)   posDefinite=',posdef,'  cofact=',cofact, &
...
60    CONTINUE
        ...
        ! if(kqt > 1) write(23,*) 'End LSQgen: nstep=',nstep

!##################################

!if(.false. .or. (compare_WTLS .and. ok .and. printG)) then
!  call test_brandt_9_10(m,n,nr,nred,list,x,y,fin)
!end if
...
!IF(printG) THEN
!  call matwrite(cx,nred,nred,23,'(150es11.3)','Matrix cx at end of LsqGen:')
!end if
RETURN
...
integer(4), INTENT(IN)          :: nr     ! number of fit parameters
real(rn), INTENT(IN)            :: eta(n) ! Vector of measurements
real(rn), INTENT(IN)            :: x(nr)  ! Vector of fit parameters
integer(4), INTENT(IN)          :: k      ! index of measurement (1 bis 30)
...
! Calling test_brandt_9_10 from with LsqGen:
!
!  IF(printG) THEN
!    call matwrite(cx,nred,nred,23,'(150es11.3)','Matrix cx at end of LsqGen:')
!  end if
!
!
!subroutine test_brandt_9_10(m,n,nr,nred,list,x,y,Gyinv)
!use UR_params,         only: rn,zero
!use Num1,              only: matwrite
!use UR_Linft,          only: covppc,Qxp,nhp
!use UR_Gleich,         only: kEGr
!
!implicit none
!
!integer(4), INTENT(IN)     :: m                 ! number of constraining equations ##############
...
!integer(4), INTENT(IN)     :: nr                ! number of parameters
!integer(4), INTENT(IN)     :: nred              ! number of parameter to be fitted (<=nr)
!integer(4), INTENT(IN)     :: list(nr)          ! fit param: 1: yes ;  2: fixed; 3: not used
!real(rn), INTENT(IN OUT)   :: x(nr)             ! vector of fit parameters
!real(rn), INTENT(IN OUT)   :: y(n)              ! vector of measurements (Y values, count rates)
!real(rn), INTENT(IN OUT)   :: Gyinv(n,n)        ! covariance matrix of y values
!
!integer(4)              :: i,k,neg
!logical                 :: ok
!real(rn)                :: dummy,Fv1
!real(rn),allocatable    :: Amat(:,:), e(:,:),Bmat(:,:),GBmat(:,:),covunk(:,:),covx_wls(:,:),Qtr(:,:),cov_wls(:,:)
!
!if(.not.allocated(e))  allocate ( e(1:m,1:n+nred) )
!if(.not.allocated(Amat)) allocate ( Amat(1:m,1:nr) )
!if(.not.allocated(Bmat)) allocate ( Bmat(1:m,1:n) )
!if(.not.allocated(GBmat)) allocate ( GBmat(1:m,1:m) )
!if(.not.allocated(Covunk)) allocate ( Covunk(1:nr,1:nr) )
!if(.not.allocated(cov_wls)) allocate ( cov_wls(1:nr,1:nr) )
!if(.not.allocated(covx_wls)) allocate ( covx_wls(1:m,1:m) )
!if(.not.allocated(Qtr)) allocate ( Qtr(1:nr,1:nr) )
!
!     call matwrite(Gyinv,n,n,23,'(50es11.3)','Matrix Cy:')
!  CALL auxdrg(x,y,m,n,nr,nred,list,e,ok,LsqGfn2)
!  ! Brandt textbook, Eq. 9.10.3:
!  do i=1,m
!    do k=1,nr
!      Amat(i,k) = e(i,k)
!    end do
!  end do
!  call matwrite(Amat,m,nr,23,'(50es11.3)','Matrix Amat, partial derivat. with respect to x vals:')
!  ! Brandt textbook, Eq. 9.10.4:
!  do i=1,m
!    do k=1,n
!      Bmat(i,k) = e(i,2+k)
!    end do
!  end do
!  call matwrite(Bmat,m,n,23,'(50es11.3)','Matrix Bmat, partial derivat. with respect to y vals:')
!  GBmat = matmul(Bmat, matmul(Gyinv, transpose(Bmat)))
!    CALL mtxchi(GBmat,m)
!  call matwrite(GBmat,m,m,23,'(50es11.3)','Matrix GBmat: Inv( Bmat * Gyinv * transp(Bmat) ); Brandt Eq. 9.10.12')
!
!  covunk = matmul(transpose(Amat), matmul(GBmat,Amat))
!    CALL mtxchi(covunk,nr)
!  call matwrite(covunk,nr,nr,23,'(50es11.3)','matrix covunk_inv: Inv( transp(Amat) * GBmat * Amat ); Brandt Eq. 9.10.19')
!
!  covx_wls = 0._rn
!  do i=1,m
!    covx_wls(i,i) = Gyinv(i*(nr+1),i*(nr+1))
!  end do
!  call matwrite(covx_wls,m,m,23,'(50es11.3)','matrix covx_wls:')
!    CALL mtxchi(covx_wls,m)
!  cov_wls = matmul(transpose(Amat), matmul(covx_wls,Amat))
!  CALL mtxchi(cov_wls,nr)
!  call matwrite(cov_wls,nr,nr,23,'(50es11.3)','matrix Transp(Amat)* covx_wls_inv * Amat:')
!
!  call matwrite(Qxp,nr,nhp,23,'(50es11.3)','matrix Qxp:')
!  Qtr = zero
!  Qtr = matmul(Qxp, matmul(covppc, transpose(Qxp)))
!  call matwrite(Qtr,nr,nr,23,'(50es11.3)','matrix Qtr: Qxp * covppc * transp(Qxp)')
!  cov_wls = cov_wls + Qtr
!  call matwrite(cov_wls,nr,nr,23,'(50es11.3)','matrix cov_wls + Qtr:')
!
!  neg = kEGr
!  Fv1 = Qtr(neg,neg)
!  write(23,*) 'neg=',int(neg,2),' Fv1=Qtr(neg,neg)=',sngl(Fv1)
!  do i=1,nhp
!    dummy = Qxp(1,i)
!    Qxp(1,i) = zero
!    Qtr = matmul(Qxp, matmul(covppc, transpose(Qxp)))
!    write(23,*) ' i=',int(i,2),' fraction of Qxp(1,i) of Qtr(neg,neg)=',sngl(Fv1-Qtr(neg,neg))
!    Qxp(1,i) = dummy
!  end do
!  do i=1,nhp
!    dummy = Qxp(2,i)
!    Qxp(2,i) = zero
!    Qtr = matmul(Qxp, matmul(covppc, transpose(Qxp)))
!    write(23,*) ' i=',int(i,2),' fraction of Qxp(2,i) of Qtr(neg,neg)=',sngl(Fv1-Qtr(neg,neg))
!    Qxp(2,i) = dummy
!  end do
!
!end subroutine test_brandt_9_10
!##########################################################################################
