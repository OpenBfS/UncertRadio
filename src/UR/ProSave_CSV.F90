...
call WandelDPkt(zahl,2)
IF(langg == 'DE') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'coverf',ctr,TRIM(zahl),ctr, &
                          'Erweiterungsfaktor für Unsicherheit (Ausgabe)',(ctr,i=1,12)
IF(langg == 'EN' .or. langg == 'FR') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'coverf',ctr,TRIM(zahl),ctr, &
  ...
  IF(langg == 'EN' .or. langg == 'FR') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'coverin',ctr,TRIM(zahl),ctr, &
                            'coverage factor for uncertainty (input)',(ctr,i=1,12)

WRITE(zahl,'(f8.6)') real(W1minusG,8)
call WandelDPkt(zahl,2)
IF(langg == 'DE') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'1-gamma',ctr,TRIM(zahl),ctr, &
                          'Wahrsch. für Konfidenz',(ctr,i=1,12)
IF(langg == 'EN' .or. langg == 'FR') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'1-gamma',ctr,TRIM(zahl),ctr, &
                          'Probability for confidence',(ctr,i=1,12)

call WDGetEntryDouble('entryOptGamDistAdd', GamDistAdd)
WRITE(zahl,'(f8.6)') real(GamDistAdd,8)
call WandelDPkt(zahl,2)
IF(langg == 'DE') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'GamDistAdd',ctr,TRIM(zahl),ctr, &
                          'Parammeter der Gamma-Verteilung',(ctr,i=1,12)
IF(langg == 'EN' .or. langg == 'FR') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'GamDistAdd',ctr,TRIM(zahl),ctr, &
                          'Gamma-distribution parameter',(ctr,i=1,12)

call WDGetSelRadiomenu('MT_NegLin',k)
if(k == 1) then
  write(zahl,'(a)') 'PosLin'
  IF(langg == 'DE') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'ModelType',ctr,TRIM(zahl),ctr, &
                             'Positiv linear, mit Nachweisgrenzen',(ctr,i=1,12)
  IF(langg == 'EN' .or. langg == 'FR') WRITE(25,'(a1,a,a1,a,a1,a,12a1)') ctr,'ModelType',ctr,TRIM(zahl),ctr, &
                             'Positive linear, with detection limits',(ctr,i=1,12)
end if
if(k == 2) then
...
IF(FitDecay) THEN
  ...
  do k=1,numd
    WRITE(text,'(i3,a1,a20,a1,10(es23.15e2,a1),3a1)') k,ctr,CStartzeit(k)%s,ctr,real(dmesszeit(k),8),ctr,  &
