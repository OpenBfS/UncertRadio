...
use UR_params,      only: eps1min,rn,zero,one,two,pi
...
INTEGER(4)         :: I,k,nng,i1,ileng,j,jk,nv,i3_1,i3_2,ij,j2,m
integer(4)         :: i2,jj,ngopsi,jpl,nnc,nwk,i5,n21,itt,nk,jmax,ir,klplast,nopj
...
do i=nab,kEGr,-1
  ...
  do j=1,len_trim(RSeiteG(1)%s)
    if(Scan(RSeiteG(1)%s(j:j),'+-*/^') > 0) then
      ...
      if(.false. .and. Scan(opsi(ngopsi),'+-') > 0) then
        write(66,'(a)') RseiteG(1)%s
        do m=opsind(ngopsi)-1,1,-1
          mch = ucase(RseiteG(2)%s(m:m))
          ! if(( mch < 'A' .or. mch > 'Z') .and. scan(mch,'0123456789.') == 0 .and. mch /=' ') then
          if(scan(mch,'0123456789.') == 0 .and. mch /= ' ') then
            read(RseiteG(1)%s(m+1:opsind(ngopsi)-1),*,iostat=ios) dummy
            if(ios == 0) then
                write(66,*) 'T1: opsind=',opsind(ngopsi),' substr=','"',RseiteG(1)%s(m+1:opsind(ngopsi)-1),'"','     m=',m
              write(RseiteG(1)%s(m+1:opsind(ngopsi)-1),'(a)') '0'
                write(66,*) 'T1a: opsind=',opsind(ngopsi),' substr=','"',RseiteG(1)%s(m+1:opsind(ngopsi)-1),'"'
            end if
            exit
          end if
        end do
        do m=opsind(ngopsi)+1,len_trim(RseiteG(1)%s),+1
          mch = ucase(RseiteG(1)%s(m:m))
          ! if(( mch < 'A' .or. mch > 'Z') .and. scan(mch,'0123456789.') == 0 .and. mch /=' ') then
          if(scan(mch,'0123456789.') == 0 .and. mch /= ' ') then
            read(RseiteG(1)%s(opsind(ngopsi)+1:m-1),*,iostat=ios) dummy
            if(ios == 0 .and. mch /= '/' .and. mch /= '*') then
                write(66,*) 'T2: opsind=',opsind(ngopsi),' substr=','"',RseiteG(1)%s(opsind(ngopsi)+1:m-1),'"','    m=',m
              write(RseiteG(1)%s(opsind(ngopsi)+1:m-1),'(a)') '0'
              write(66,*) 'T2a: opsind=',opsind(ngopsi),' substr=','"',RseiteG(1)%s(opsind(ngopsi)+1:m-1),'"'
            end if
            exit
          end if
        end do
  ...
  do k=1,nRSsy(i)
    ...
    if(.false. .and. scan(RseiteOrg(jk:jk),'+-') == 0) then
      ceinhwks = '(' // trim(EinheitWK(nng)%s) // ')'
      ceinhwk  = ceinhwks
      do ij=1,UU%nsymb
        if(einhVal(ij) >= 1.0_rn) then
          write(cnum,'(f6.1)') einhVal(ij)
        else
          write(cnum,'(f10.6)') einhVal(ij)
        end if
        cnum = trim(adjustL(cnum))
        call StrReplace(ceinhwk,trim(ucase(UU%EinhSymb(ij)%s)),trim(cnum),.true.,.true.)
      end do
                       !write(66,*) ' before do loop, goto 122:  ceinhwks=',trim(ceinhwks), &
...
57   continue
  ...
  do j=1,UU%nSymb
    ...
    if(abs((Fv2-Fv1)/dpa) > 1.E-8_rn*Evalue) then
      ...
      !if(prout) write(66,'(2(a,i0),a,a,2(a,es16.9))') 'NOT:  nv=',nv,' j=',j, &
      !                  ' UU%EinhSymb(j)=',UU%EinhSymb(j)%s,' dpi=',(dummy-Evalue)/dpa, &
      !                  ' dummy=evalf=',dummy
    end if
  end do
...
integer(4),intent(out)        :: jj         ! number of the basic unit
integer(4),intent(out)        :: jpl        ! number added to jj
...
use UR_params,      only: eps1min,rn,zero,one,two
...
integer(4),intent(in)           :: ie       ! equation number
...
integer(4),intent(in)           :: n21      ! how often is the unit "s" contained in Rstring

integer(4)          :: i0,i1,i2,i3,jplus,jk,ios
real(rn)            :: dummy,seval
logical             :: fail
...
integer(4),intent(out)       :: nf        ! number of occurences of sfunc in Rstring
integer(4),intent(out)       :: i1arr(*),i2arr(*),i3arr(*)

integer(4)          :: i0,i1,i2,i3,jplus,jk,ii,klauf
...
integer(4),intent(inout)  :: kper
integer(4),intent(out)    :: arr2dim(2**n,n)
...
integer(4),intent(out)   :: arr(*)
integer(4),intent(inout) :: kper

integer(4)          :: j,k
integer(4)          :: arr2dim(2**n,n)
...
use UR_Gleich,         only: ifehl,nab,ngrs,Symbole,EinheitSVUCH,Einheit,uconv, &
