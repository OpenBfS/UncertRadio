...
!                                 % 3: Nielsen's lambda update equations
!
!         Translation from Matlab into Fortran by Günter Kanisch, Hamburg, 2024
!
!-------------------------------------------------------------------------------------
...
integer(4),intent(in)   :: npnt,npar, maxiter,kfit(npar),kfitmeth2      ! prnt,
real(rn),intent(in)     :: t(npnt),y_dat(npnt),uy_dat(npnt)
integer(4),intent(out)  :: iteration
...
integer(4),intent(in)   :: ipr,jpr
logical,intent(out)     :: convg

integer(4)      :: i,func_calls,m,n               ! global
integer(4)      :: Update_Type,irunPLSQ,Nfit,k1,nsumv,neq,iratmax,kqt
  ...
  if ( iteration == MaxIter ) then
    if(ipr >= 2) &
      write(jpr,'(a,a,i0,a,L1,a,L1,a,i0)') '!!Maximum Number of Iterations Reached Without Convergence !!', &
                        '  kqt=',int(kqt,2),' upropa_on=',upropa_on,' MCsim_on=',MCsim_on, &
                        ' meth(+1)=',kfitmeth2
    pstop = .true.
  end if
...
integer(4),intent(in)   :: npnt,npar,kfitmeth2,kfit(npar)     ! number of data points; number of parameters; fit method
...
USE UR_Params,     only: rn,zero,one,two
use UR_Linft,      only: xB,ifit,iap,d0zrate,dmesszeit,RBGmean,mfRBG_fit_PMLE
use UR_MCSR,       only: mw_rbl
