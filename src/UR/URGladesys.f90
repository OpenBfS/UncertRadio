...
subroutine URGladeSys(idatei)
    ...
    ! of 45 keywords, packed into the array oclass (object class).
    !
    ...
    ! The routine Strgencode is used for preparing a copy of the original Glade file,
    ! in which the lines are written in encrypted format.
    !
    !     Copyright (C) 2014-2023  Günter Kanisch

use UR_gtk_variables
use, intrinsic :: iso_c_binding
use UR_interfaces,     only: Strgencode,PrintGladeSys
use CHF,               only: ucase
...
integer(4), intent(in)         :: idatei      ! 1: Original-Glade file; 2: encrypted Glade file - delivered

character(len=120)  :: str
character(len=230)  :: text,textSV,search,tarr(5),textcd,textbg,textin
...
clobj%idparent = 0
    ...
    ! do i=1,255
    !   write(66,'(a,i4,a,a1)') 'i=',i,' char=',char(i)
    ! end do
...
!Notebook2_labelid(1) ='nb2LabelMC'
!Notebook2_labelid(2) ='nb2LabelMCMC'
!Notebook2_labelid(1) ='nb2LabelOther'
...
call stat(Gladeorg_file,finfo)
  ! write(66,*) 'GLSys ------------  File=',trim(GLadeorg_file),'  last modif date=',finfo(10),'  time=',CTIME(finfo(10))
call stat(Gladedec_file,finfo)
  ! write(66,*) 'GLsys ------------  File=',trim(Gladedec_file),'  last modif date=',finfo(10),'  time=',CTIME(finfo(10))
  ! write(66,*) 'GLsys ------------  idatei=',idatei
  ! write(66,*) 'GLsys ------------  glade_org=',glade_org,'  glade_dec=',glade_dec

close (18)
if(idatei == 1) open(18,file=gladeorg_file,status='old',iostat=ios,iomsg=messg)
if(idatei == 2) open(18,file=gladedec_file,status='old',iostat=ios,iomsg=messg)

if(ios /= 0) then
    ifehl = 1
  if(idatei == 1 ) write(66,*) 'Error on opening the Glade file ',trim(gladeorg_file),' ios=',int(ios,2),' ',trim(Messg)
  if(idatei == 2 ) write(66,*) 'Error on opening the Glade file ',trim(gladedec_file),' ios=',int(ios,2),' ',trim(Messg)
  return
end if
...
do i=1,100000
  ...
  text = trim(textin)
     ...
     if(idatei == 1) then
       call strgencode(keystrg,textin,textcd,1, i)
       write(19,'(a)') trim(textcd)

       call strgencode(keystrg,textcd,textbg,2, i)
       write(20,'(a)') trim(textbg)
         if(len_trim(textin) /= len_trim(textcd)) write(66,*) 'GladeSys: Warnung: back /= in'
     end if
     if(idatei == 2) then
       call strgencode(keystrg,textin,text,2, i)
       ! write(66,*) 'text=',text
     end if
...
do i=1,100000
  ...
  text = trim(textin)
  if(idatei == 2) call strgencode(keystrg,textin,text,2, i)
...
use UR_gtk_variables, only: nstores,storename,tvmodel,lscolnums,lstype,lsgtype,  &
                            tvcolindex,ntvs
implicit none
...
integer(4)           :: jc, jt, kc,klen
character(len=400)   :: textw
! type(c_ptr)         :: resp

! Note:
! Do not use German "Umlaute" in the original Glade file! Otherwise,
! errors may occur by using this routine for encoding the Glade text lines.

klen = len_trim(keystrg)
textw = textin
   !if(mode == 1 .and. index(textw,'ä')> 0) then
   !  write(97,'(a)') trim(textw)
   !  resp = g_locale_to_utf8(trim(textw)//c_null_char, len_trim(textw), c_null_ptr,c_null_ptr,c_null_ptr)
   !  write(97,'(a)') trim(textw)
   !end if
...
if(mode == 1) then
  ...
  textout = ''
  jc = 0
  do jt=1,len_trim(textw)
    jc = jc + 1
    if(jc > klen) jc = jc - klen
    kc = ichar(textw(jt:jt))+ichar(keystrg(jc:jc))
    if(kc > 255) kc = kc - 240         ! with 240 the EOL characters LF or CR are excluded!
    textout(jt:jt) = char(kc)
      !if(isatz == 6910 .and. jt == 82) then
      !   write(66,*) ' 6910:  Zeichen 82: ',textin(jt:jt),' ',ichar(textin(jt:jt)), &
      !               ' codez=',keystrg(jc:jc),' jc=',jc,' ichar(keystrg(jc:jc))=',ichar(keystrg(jc:jc)), &
      !               '  sum kc=',kc,'  zout: ',char(kc)
      !end if
  end do
elseif (mode == 2) then
  ! decode:  d
  textout = ''
  jc = 0
  do jt=1,len_trim(textin)
    jc = jc + 1
    if(jc > klen) jc = jc - klen
    kc = ichar(textin(jt:jt))-ichar(keystrg(jc:jc))
    if(kc < 0) kc = kc + 240
    textout(jt:jt) = char(kc)
  end do
