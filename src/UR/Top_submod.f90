  ...
  do i=1,nclobj
    if(c_associated(ptr, clobj%id_ptr(i))) then
      ...
      exit
         ...
         ! write(66,*) 'IDPT:  ',trim(strid),'   ptr=',ptr
...
!#############################################################################################
!
  module SUBROUTINE FieldUpdate(string)
  !
  ...
  character(len=*),intent(in)  ::  string
           ! if(savep) write(66,*) 'Fieldupdate: ',trim(string),'  saveP=T'
  select case (Filetyp)
    case ('P')
      IF (SAVEP) THEN
...
!#######################################################################
  ...
  module subroutine LTU(string)
    ...
    !  this routine translates a character string from local to UTF8 coding.

    !     Copyright (C) 2014-2023  Günter Kanisch

  use, intrinsic :: iso_c_binding,       only: c_ptr,c_null_ptr,c_null_char,c_associated,c_int64_t
  ...
  type(c_ptr)                 :: cdummy1,cdummy2,resp
  ...
  allocate(character(len=len_trim(string)+200) :: str)

  if(prout) then
    write(0,*) 'LTU input ======================================'
    write(0,*) 'string=',trim(string)
    ! write(66,*) 'LTU input ======================================'
  end if

  if(len_trim(string) == 0)  return
  n = len_trim(string) + 30
  nc = n
  ! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ! the following single statement seemed to have removed the "randomly occurring" errors:
  string = trim(string) // '                              '   !
  ! This made the actual string length and the value of n always being identical
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  cerror = c_null_ptr !FO
  resp = g_locale_to_utf8(string//c_null_char, nc, c_null_ptr, c_null_ptr, cerror) !FO
  if(c_associated(resp)) call c_f_string(resp, str)
  if(.not.c_associated(resp)) str = string

  string = trim(str)

  if(prout) then
    write(0,*) 'LTU:  resp=',resp,' error=',cerror
    write(0,*) 'LTU output ======================================'
    write(0,*) trim(string)
    ! write(66,*) 'LTU output ======================================'
    endif
...
!---------------------------------------------------------------------------------------------
  ...
  module subroutine LFU(string)
    ...
    !  this routine translates a UTF8 character string to local coding.

    !     Copyright (C) 2014-2023  Günter Kanisch

  use, intrinsic :: iso_c_binding,       only: c_ptr,c_null_ptr,c_null_char,c_associated,c_size_t,c_loc
  use gtk_sup,             only: c_f_string
  ...
  character(:),allocatable    :: str
  type(c_ptr)                 :: resp,cbytes_read,cbytes_written
  ...
  len = len_trim(string)
  cerror = c_null_ptr
  cbytes_read = c_null_ptr
  resp = g_locale_from_utf8(trim(string)//c_null_char, len, cbytes_read,cbytes_written,c_loc(cerror))
  if(c_associated(resp)) then
    call c_f_string(resp,str)
      ! write(0,*) ' LFU:    lentrim(string)=',len_trim(string),'  len_trim(LFU)=',len_trim(str)
  else
     str = ' '
  end if
    string = trim(str)
...
!#######################################################################
  ...
  use UR_Gleich,      only: nvalsMD,meanMD,umeanMD,fbayMD,k_MDtyp,nvarsMD, &
                            nvMD,ifehl,smeanMD,DistPars,MDpoint,nmxDist,Symbole, &
...
integer(4)          :: i,ix1,ix2,j,mix1,mix2,k
character(len=20)   :: sbuf
...
use UR_Gleich,   only: apply_units, unit_conv_fact,ngrs,einheit,nab,ncov, &
                       Symbole,Messwert,einheit_conv,UU,unit_conv_factSV
use CHF,         only: ucase

implicit none

integer(4),intent(in)   :: nvars    ! number of variables; at the moment: ngrs+ncov

integer(4)          :: i,j,k,nc,klast,k2,klen,nfd,iexp
