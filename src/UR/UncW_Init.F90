...
USE UR_Gleich,        only: DistPars,apply_units,apply_units_dir,coverf,coverin,cpu_topo, &
                            ...
                            kbrutto_double,kbrutto_name,knetto_name,symb_kEGr,ndep,use_sdf_brutto, &
...
use UR_gtk_variables, only: toggleTypeGTK,dialogloop_on,NBsoftSwitch,list_filling_on, &
                            ...
                            TV1_lentext,dialog_on,switched_ignore,runauto,&
...
integer(4)           :: i1,ios,i,j
character(len=:),allocatable   :: text, textG
character(len=20)    :: cvalue
logical              :: prfound
type(c_ptr)          :: ctxt
integer(c_int)       :: resp
character(len=100)   :: dlangg,locale_strg,errmsg
real(rn)             :: windowRelSize
...
integer(4)        :: j,kwd,kht,pixel_per_char,n,cwidth,nt
