...
USE UR_params,      ONLY: rn,Pi,eps1min,zero,one,two,zero
...
integer(4)            :: i,j,k,ksav,klu,kk
integer(4)            :: resp,ndd,itest
real(rn)              :: dummy,omega
...
integer(4)               :: kzero1,kone1,imax,kfound
real(rn)                 :: w,urelw1,urelw2,varw,fv1,fv2,dpa,dpi
real(rn),allocatable     :: Bmat(:,:), dvec(:),bvec(:),Uxinv(:,:)
integer(4),allocatable   :: iact(:),iter(:)
!-----------------------------------------------------------------------
...
if(ifehl == 1) goto 9000
  ...
  do i=1,ngrs+ncov+numd
   ...
   ! WRITE(66,'(a,2x,T30,2(2x,a,es15.8))') Symbole(i)%s,' Messwert=',real(Messwert(i),8),  &
...
do itest=1,2
  ...
  WRITE(30,'(/,1x,a,a,a,i2)') 'procedure : ',verfahren
  dummy = zero
...
use UR_params,         only: eps1min,zero,one,two,three
...
integer(4)       :: klu, iter,mmqt,itmax,modeSV
